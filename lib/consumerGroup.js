    ...
    [
      function (callback) {
        if (typeof self.options.onRebalance === 'function') {
          ...
          self.options.onRebalance(self.memberId != null, function (error) {
            if (error) {
              return callback(error);
            }
            callback(null);
      ...
      function (coordinatorInfo, callback) {
        ...
        self.pendingJoin = new Promise((resolve) => {
...
ConsumerGroup.prototype.prepareClose = function (requestTimeout) {
  if (this.closing) {
    return;
  }
  this.closing = true;
  ...
  this.client.prepareClose(requestTimeout);
};
