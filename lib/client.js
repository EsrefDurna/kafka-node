...
Client.prototype.prepareClose = function (requestTimeout) {
  // Make any dead connections die faster
  const changeTimeout = (broker) => {
    const socket = broker.socket;
    if (socket) {
      socket.setTimeout(requestTimeout != null ? requestTimeout : 30000);
...
Client.prototype.sendToBroker = function (payloads, encoder, decoder, cb) {
  ...
  for (var leader in payloads) {
    ...
    try {
      ...
      logger.error('error getting broker', e);
    }
...
Client.prototype.createBroker = function (host, port, longpolling) {
  ...
  var brokerKey = socket.host + ':' + socket.port;
...
Client.prototype.reconnectBroker = function (oldSocket) {
  ...
  var brokerKey = oldSocket.host + ':' + oldSocket.port;
  // Someone else may of triggered a new broker connection, don't overwrite it
  if (!brokers[brokerKey]) {
    this.setupBroker(oldSocket.host, oldSocket.port, oldSocket.longpolling, brokers);
  }
};
