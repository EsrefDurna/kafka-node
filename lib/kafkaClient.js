...
KafkaClient.prototype.connect = function () {
  ...
  connect.attempt(currentAttempt => {
    ...
    async.series(
      ...
      error => {
        if (!this.closing && connect.retry(error)) {
          return;
        }
        ...
        if (error) {
          if (this.options.autoConnect && !this.closing) {
            this.connect();
          }
...
KafkaClient.prototype.connectToBroker = function (broker, callback) {
  ...
  const brokerWrapper = this.getBroker(broker.host, broker.port, false);
...
KafkaClient.prototype.prepareClose = function (requestTimeout) {
  // Make any dead connections die faster
  const changeTimeout = (broker) => {
    const socket = broker.socket;
    if (socket) {
      socket.setTimeout(requestTimeout != null ? requestTimeout : this.options.requestTimeout);
      if (socket.readyState === 'open') {
        keepAlive.setKeepAliveProbes(socket, 5);
      }
    }
  };

  _.each(this.brokers, changeTimeout);
  _.each(this.longpollingBrokers, changeTimeout);
};
...
KafkaClient.prototype.createBroker = function (host, port, longpolling) {
  ...
  socket.on('timeout', function () {
    console.log("kafka-broker: BrokerTimeout " + readyEventName);
  ...
  socket.on('connect', function () {
    keepAlive.setKeepAliveInterval(socket, 1000);
    keepAlive.setKeepAliveProbes(socket, 10);
    ...
    if (lastError) {
      ...
      self.initializeBroker(brokerWrapper, function (error) {
        console.log("kafka-broker: BrokerReconnect " + readyEventName, error);
        self.emit(readyEventName, error);
        if (error) {
      ...
      self.initializeBroker(brokerWrapper, function (error) {
        console.log("kafka-broker: BrokerConnect " + readyEventName, error);
        self.emit(readyEventName, error);
        if (error) {
  ...
  socket.on('error', function (err) {
    console.log("kafka-broker: BrokerError " + readyEventName, err);
    socket.error = err;
    cleanUp();
    if (!self.connecting) {
  ...
  socket.on('close', function () {
    console.log("kafka-broker: BrokerClose " + readyEventName);
  ...
  socket.setKeepAlive(true, this.options.connectTimeout);
  ...
  function cleanUp () {
    ...
    if (!error) {
      if (self.options.sasl && socket.saslAuthCorrelationId !== undefined) {
        ...
        if (!self.connecting && !brokerWrapper.isIdle() && !self.closing && !socket.closing) {
    ...
    self.clearCallbackQueue(socket, error);
    if (self.closing) {
      self.emit(readyEventName, error);
    }
    delete self.brokers[brokerKey];
    delete self.longpollingBrokers[brokerKey];

    socket.retryTimer = setTimeout(function () {
      if (self.closing || socket.closing) {
        // If anyone is waiting for this broker, they need to know there is an error
        self.emit(readyEventName, socket.error || new errors.ClientIsClosing());
        return;
      }
      const socketId = socket.socketId;

      const queue = self.cbqueue.get(socketId);
      if (brokerWrapper.isIdle() && !(queue && queue.size)) {
        logger.debug(`${self.clientId} to ${socket.addr} is idle not reconnecting`);
        socket.closing = true;
        return;
      }

      if (!self.isValidBroker(socket)) {
        logger.debug(`${self.clientId} is not reconnecting to ${socket.addr} invalid broker`);
        return;
      }
...
KafkaClient.prototype.deleteDisconnected = function (broker) {
  if (!broker.isConnected()) {
    ...
    const key = broker.socket.host + ':' + broker.socket.port;
    if (brokers[key] === broker) {
      delete brokers[key];
    }
  }
};
...
KafkaClient.prototype.waitUntilReady = function (broker, callback) {
  const onReady = (err) => {
    ...
    callback(err || (this.closing ? new errors.ClientIsClosing() : null));
  };
