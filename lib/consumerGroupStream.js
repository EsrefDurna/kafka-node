...
class ConsumerGroupStream extends Readable {
    ...
    this.commitPendingPromise = new Promise((resolve, reject) => {
      ...
      const processOffsetCommitResponse = (error, topics) => {
        ...
        if (isTimeout && hasRetried && this.options.treatTimeoutsAsSuccess) {
          ...
          console.error('kafka-broker: commit request failed', error);
            ...
            Object.keys(partitions).forEach(/** @type number */partition => {
              ...
              if (callbacks && callbacks.length) {
                ...
                if (isReplicationTimeout && hasRetried && this.options.treatTimeoutsAsSuccess) {
                  console.warn('kafka-broker: 2 timeout errors in a row, treating as success due to user choice');
                ...
                console.error('kafka-broker: partition error with no callback receiving it', partitionError);
        ...
        if (shouldRetry) {
          hasRetried = true;
          console.warn('kafka-broker: retrying offset commit as some partitions timed out');
          setTimeout(() => {
            ...
            this.consumerGroup.getOffset().fetch(payloads, (error, result) => {
              if (error) {
                if (this.consumerGroup.client.closing) {
                  processOffsetCommitResponse(new TimeoutError('Offset commit timed out but the client is closing, and got another error: ' + error.message));
                  return;
                }
                ...
                console.error('kafka-broker: got error on retry for offset commit, retrying in hopes it will succeed');
              ...
              if (partitionCallbacks.size) {
                if (this.consumerGroup.client.closing) {
                  processOffsetCommitResponse(new TimeoutError('Offset commit timed out but the client is closing'));
                  return;
                }
  ...
  prepareClose (requestTimeout) {
    this.consumerGroup.prepareClose(requestTimeout);
  }

  close (callback) {
    this.prepareClose(this.options.requestTimeout);
